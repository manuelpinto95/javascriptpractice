...
const validateCred = (arr) => {
  ...
  sum = arr[numberOfDigits - 1];
  parity = (numberOfDigits - 2) % 2;
...
const findInvalidCards = (array) => {
  ...
  for (let card of array) {
    if (!validateCred(card)) { resultInvalid.push(card); }
  }
  return resultInvalid;
};

//Find all invalid companies in a batch. Remove duplicates
//NOTE: I should have probably implemented an object that enumerates what number belongs to which card...
  ...
  for (let card of array) {
    if (!validateCred(card)) {
      switch(card[0]) {
        ...
        default:
          if (!companiesArray.includes('Company not found')) { companiesArray.push('Company not found'); } 
      }
    }
  }
